...
import {
  ...
  listBlogContentType,
  addBlogContentType,
} from "../../data/knowledgeCenterManagement";
...
function AddEditKnowledgeBlog(props) {
  ...
  let handleUpload = (e) => {
    ...
    setMediaInput({
      ...
      isError: false,
      source:
        "https://changinghabits-dev-backend.s3.amazonaws.com/changinghabits/learning_content/loading-buffering_1625498388794.gif",
    });
    if (e.target.files[0]) {
      ...
      if (fileType != "image" && fileType != "video" && fileType != "audio") {
        setMediaInput({
          ...
          source: null,
          isError: true,
          errorMessage: "Only image/video/audio file allowed",
        });
        return;
      }
  ...
  let updateContentList = (data = {}) => {
    ...
    listBlogContentType()
      .then((response) => {
        setContentTypeList(
          response.blogContentType.map((content) => {
            return {
              ...content,
              value: content.id,
        ...
        if (data.content_name) {
          ...
          let newContent = response.blogContentType.find(
            (content) => content.content_name == data.content_name
          );
          if (newContent) {
    ...
    if (params.id) {
      ...
      getBlog(req)
        .then((response) => {
          ...
          setContentTypeCheck(false);
          if (
            response.blogDetails.image_url ||
            response.blogDetails.video_url ||
            response.blogDetails.audio_url
          ) {
            setMediaInput({
              type: response.blogDetails.image_url
                ? "image"
                : response.blogDetails.video_url
                ? "video"
                : "audio",
          ...
          <CCard>
            <CCardHeader>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <h2>
                  {history.location.pathname == "/addKnowledgeBlog"
                    ? "Add Knowledge Blog"
                    : "Edit Knowledge Blog"}
                  <CSpinner
                    style={{
                  ...
                  <CFormGroup style={{ width: "45%" }}>
                    ...
                    <div>
                      ...
                      <div
                        style={{
                          marginTop: "0.5rem",
                          display: showAddContentInput ? "none" : "",
                        }}
                      >
                        <CTooltip
                      ...
                      >
                        ...
                        <div
                          style={{
                            color: "red",
                            display: newContentTypeCheck ? "" : "none",
                          }}
                        >
                          New content type name is required
                        </div>
                      </div>
                    </div>
                  </CFormGroup>
