...
function AddEditFoodLogSuggestion() {
  ...
  let handleAddInputGroup = () => {
    ...
    currentInputGroups.push({
      inputGroup_no:inputGroups.length+1,phase:0,phaseCheck:false,categoryList:[],category:0,categoryCheck:false,weekList:[],week:0,
    })
    setInputGroups(currentInputGroups);
  }
  ...
  let validateField = () => {
    ...
    if(!quantityFrom.value){
      ...
      result=false;
    }else if( quantityTo.value && !isNaN(quantityTo.value) && parseFloat(quantityFrom.value)>=parseFloat(quantityTo.value)){
      setQuantityFrom({
        ...quantityFrom,
        check:true,
    ...
    if(!quantityTo.value){
      ...
      result=false;
    }else if(quantityFrom.value && !isNaN(quantityFrom.value) && parseFloat(quantityTo.value)<parseFloat(quantityFrom.value)){
      setQuantityTo({
        ...quantityTo,
        check:true,
            ...
            <CCard>
              ...
              <CCardBody>
                ...
                <CForm action="" method="post" onSubmit={handleSubmit}  autoComplete="off">
                  {inputGroups.map((inputGroup,index)=>{
                    return(
                      ...
                      <CFormGroup style={{width:"30%"}}>
                          ...
                          <CSelect
                        onChange={(e) =>handleChangeInputGroupValue(index,e,"phase")}
                    ...
                    <div style={{color:"red",marginLeft:"0.1rem", display:inputGroup.phaseCheck?"":"none"}}>Phase is required</div>
                    </CFormGroup>
                    <CFormGroup  style={{width:"30%"}}>
          
                          <CLabel style={{fontWeight:"600",fontSize:"1rem"}} htmlFor="category">Category</CLabel>
                          <CSelect
                      onChange={(e) => handleChangeInputGroupValue(index,e,"category")}
                    ...
                    <div style={{color:"red",marginLeft:"0.1rem", display:inputGroup.categoryCheck?"":"none"}}>Category is required</div>
                    </CFormGroup>
                    <CFormGroup  style={{width:"30%"}}>
                          ...
                          <div style={{display:"flex",justifyContent:"space-between"}}>
                          <CSelect
                      onChange={(e) => handleChangeInputGroupValue(index,e,"week")}
                    ...
                    <div style={{display:inputGroups.length>1?"":"none"}}>                              
                              <CBadge style={{marginLeft:"0.5rem",marginTop:"0.5rem", cursor:"pointer"}} color="danger" onClick={()=>handleRemoveInputGroup(index)}><FaMinus/></CBadge>
                      </div>
                      </div>
                      </CFormGroup>
                  ...
                  <CBadge
                    style={{ marginBottom:"1rem",  cursor: "pointer", display:params.id?"none":"", }}
                    color="secondary"
                  ...
                  <div style={{display:"flex", justifyContent:"space-between"}}>
                      <CFormGroup style={{width:"30%"}}>
                          ...
                          <CLabel style={{fontWeight:"600",fontSize:"1rem"}} htmlFor="quantity_from">Quantity From</CLabel>
                          <CInput
                      onChange={(e) => {
                        setQuantityFrom({
                          ...quantityFrom,
                      ...
                      //required
                    />
                    <div style={{color:"red",marginLeft:"0.1rem", display:quantityFrom.check?"":"none"}}>{quantityFrom.error}</div>
                    </CFormGroup>
                    <CFormGroup  style={{width:"30%"}}>
          
                          <CLabel style={{fontWeight:"600",fontSize:"1rem"}} htmlFor="quantity_to">Quantity To</CLabel>
                          <CInput
                      onChange={(e) => {
                        setQuantityTo({
                          ...quantityTo,
                      ...
                      //required
                    />
                    <div style={{color:"red",marginLeft:"0.1rem", display:quantityTo.check?"":"none"}}>{quantityTo.error}</div>
                    </CFormGroup>
                    <CFormGroup  style={{width:"30%"}}>
          
                          <CLabel style={{fontWeight:"600",fontSize:"1rem"}} htmlFor="quantity_unit">Quantity Unit</CLabel>
                    ...
                    <div style={{color:"red",marginLeft:"0.1rem", display:quantityUnitCheck?"":"none"}}>Quantity unit is required</div>
                      </CFormGroup>
                                </div>
