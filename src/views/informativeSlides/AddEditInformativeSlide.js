...
function AddEditInformativeSlide(props) {
  ...
  let [imageCheck, setImageCheck] = useState(false);
    ...
    if (e.target.files[0]) {
      ...
      if (fileType != "image") {
        setMediaInput({
          ...
          source: null,
          isError: true,
          errorMessage: "Only image file allowed",
        });
        return;
      }
  ...
  let updateSectionList = (data = null) => {
    ...
    getSectionList()
      .then((response) => {
        ...
        setSectionList(
          response.slideCategoryList.map((section) => {
            return {
              ...section,
              value: section.id,
        ...
        if (data) {
          ...
          let addedSection = response.slideCategoryList.find(
            (item) => item.slide_category_name == data.slide_category_name
          );
          if (addedSection) {
            setSection({
              ...addedSection,
      ...
      getSlide(req)
        .then((response) => {
          ...
          if (!section) {
            setSection(
              sectionList.find(
                (item) => item.id == response.slideDetails.section_id
              )
            );
          }
          setSectionCheck(false);
          ...
          <CCard>
            <CCardHeader>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <h2>
                  {history.location.pathname == "/addInformativeSlide"
                    ? "Add Informative Slide"
                    : "Edit Informative Slide"}
                  <CSpinner
                    style={{
                      color: "#008080",
                      marginLeft: "2rem",
                ...
                <CFormGroup>
                  ...
                  <div>
                    ...
                    <div
                      style={{
                        marginTop: "0.5rem",
                        display: showAddSectionInput ? "none" : "",
                      }}
                    >
                      <CTooltip
                    ...
                    >
                      ...
                      <div
                        style={{
                          color: "red",
                          display: newSectionCheck ? "" : "none",
                        }}
                      >
                        New section name is required
                      </div>
                    </div>
                  </div>
                </CFormGroup>
