  ...
  const getData = async () => {
    try {
      ...
      for (let item of updatedData) {
        let slideGorupBySection = updatedData.filter(
          (data) => data.section_id == item.section_id
        );
        item.maxOrder = slideGorupBySection.reduce(
          (max, data) => (data.order > max ? data.order : max),
          slideGorupBySection[0].order
        );
      }
      setData([...updatedData]);
                  ...
                  <CCol style={{ marginBottom: "1rem", display: "flex" }}>
                    <CInputGroup>
                      ...
                      <CInput
                        ...
                        id="search"
                        name="search"
                        placeholder="Search by name, slide no"
                        value={searchValue}
                        onChange={(e) => {
                        ...
                        >
                          ...
                          >
                            <CIcon
                              ...
                              name={"cilPencil"}
                              onClick={() =>
                                history.push(`/editInformativeSlide/${item.id}`)
                              }
                            />
                          </CTooltip>
                          <CTooltip
                          ...
                          >
                            <FontAwesomeIcon
                              ...
                              onClick={() =>
                                item.order > 1 && section > 0
                                  ? handleSlideOrderChange(item, "up")
                                  : ""
                              }
